...
function SkillDamageProc(aIndex,bIndex,skill,count,combo,InitDamage)
	...
	or skill == SKILL_CHARGE_SLASH 
	or skill == SKILL_WINDGLAIVE 
	or skill == SKILL_BLADESTORM
	or skill == SKILL_ILLUSION_AVATAR_ATTACK
...
function DamageVisualSplit(aIndex,bIndex,count,skill,effect)
	...
	elseif(skill == SKILL_RAINING_ARROW or skill == SKILL_HOLY_BOLT) then
		bVal = (count%3) -- (((count % 3) == 0) ? 0x20 : 0x10);
	elseif(skill == SKILL_CHARGE_SLASH or skill == SKILL_WINDGLAIVE or skill == SKILL_ILLUSION_AVATAR_ATTACK) then
...
function SkillIllusionBlade(aIndex,SkillIndex) -- SKILL_ILLUSION_BLADE

	local value1 = 110
	
	local value2 = GetMasterSkillValue(aIndex,MASTER_SKILL_ADD_SKILL_ILLUSION_BLADE_IMPROVED)
	
	local value3 = GetMasterSkillValue(aIndex,MASTER_SKILL_ADD_SKILL_ILLUSION_BLADE_ENHANCED)
	
	--value = value + GetMasterSkillValue(aIndex,MASTER_SKILL_ADD_SKILL_ILLUSION_BLADE_IMPROVED);

	local count = 300;

	AddEffect(aIndex,0,GetSkillEffect(SkillIndex),count,value1,value2,value3,0, 0, 0, 0, aIndex, -1);
