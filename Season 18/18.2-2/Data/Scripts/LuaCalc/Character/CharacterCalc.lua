...
function AutoRecuperationHP(aIndex,mode)

	if(mode == 0 and CheckEffect(aIndex, EFFECT_PARALYSIS) == 1) then
		return 0
	end
	
	local lpObj = GetUserObject(aIndex);
	local Class = lpObj.Class;
	local rest = lpObj.Rest;	-- is acction sit, heal,etc
	local rate = 0.0;
	local recoveryLife = 0.0;
	
	if(Class == CLASS_DW) then
		if(rest ~= 0 and mode == 0) then
			rate = 3.0
		else
			rate = 0.0
		end
	...
	elseif(Class == CLASS_IK) then
		...
		LogAdd(LOG_RED,string.format('[AutoRecuperationHP] Class Unk :%d', Class))
	end
	
	if(mode == 0) then
		if((GetTickCount()-lpObj.HPAutoRecuperationTime)>5000) then
			rate = rate + 5
		end
	end
	
	rate = rate + lpObj.HPRecoveryRate + lpObj.EffectOption_AddHPRecoveryRate;
	
	if(rate == 0.0) then
		return 0.0
	end
	
	local recoveryLife = (lpObj.MaxLife + lpObj.AddLife) * rate / 100.0;
	recoveryLife = recoveryLife + lpObj.HPRecovery + lpObj.EffectOption_AddHPRecovery;
		
	return recoveryLife
end

function AutoRecuperationSD_OLD(aIndex,mode)

	local lpObj = GetUserObject(aIndex);
	local Class = lpObj.Class;
	local rate = 0.0;
	local recoverySD = 0.0;
	...
	elseif(Class == CLASS_IK) then
		...
		LogAdd(LOG_RED,string.format('[AutoRecuperationSD] Class Unk :%d', Class))
	end
	
	if(mode == 0) then
		if((GetTickCount()-lpObj.SDAutoRecuperationTime)>5000) then
			rate = rate + 5
		end
	end
	
	rate = rate + lpObj.SDRecoveryRate + lpObj.SDRecoverySpeedRate + lpObj.EffectOption_AddSDRecoveryRate;
	
	if(rate == 0.0) then
		return 0.0
	end
	
	recoverySD = recoverySD + ((lpObj.MaxShield + lpObj.AddShield) * rate / 100.0);
	recoverySD = recoverySD + lpObj.SDRecovery + lpObj.EffectOption_AddSDRecovery;
	
	return recoverySD
end

function AutoRecuperationSD(aIndex,mode)

	local lpObj = GetUserObject(aIndex);
	local Class = lpObj.Class;
	
	local ExprecionA = (lpObj.MaxShield + lpObj.AddShield) / 30;
	local ExprecionB = (lpObj.SDRecoveryRate + lpObj.EffectOption_AddSDRecoveryRate) + 100;
	local ExprecionC = ((ExprecionA * ExprecionB) / 100) / 25;
	
	local recoverySD = 0;
	
	if(mode == 0) then
		local tTime = (GetTickCount()-lpObj.SDAutoRecuperationTime);
		
		if (tTime >= (25 * 1000)) then
			recoverySD = ExprecionC * 3;
		elseif (tTime >= (15 * 1000)) then
			recoverySD = (ExprecionC * 25) / 10;
		elseif (tTime >= (10 * 1000)) then
			recoverySD = ExprecionC * 2;
		else
			return 0.0
		end
	else
	...
	recoverySD = recoverySD + 1
	
	if(recoverySD == 0.0) then
		return 0.0
	end
	
	recoverySD = recoverySD + lpObj.SDRecovery + lpObj.EffectOption_AddSDRecovery;
	recoverySD = (recoverySD * lpObj.SDRecoverySpeedRate / 100) + recoverySD
	
	return recoverySD
end

function AutoRecuperationMP(aIndex,mode)
	...
	if(mode == 0) then
		if((GetTickCount()-lpObj.MPAutoRecuperationTime)>5000) then
			rate = rate + (3 / 100)
		end
	end
	
	rate = rate + (lpObj.MPRecoveryRate / 100);
	rate = rate + (lpObj.EffectOption_AddMPRecoveryRate / 100);
	
	if(rate == 0.0) then
		return 0.0
	end
	
	local recoveryMana = ((lpObj.MaxMana + lpObj.AddMana) * rate);
	recoveryMana = recoveryMana + lpObj.MPRecovery + lpObj.EffectOption_AddMPRecovery;

	return recoveryMana
end

function AutoRecuperationBP(aIndex,safe,mode)
	...
	local Class = lpObj.Class;
	local AddBPPoint = 2;
	local MaxBP = lpObj.MaxBP + lpObj.AddBP;
	local recoveryBP = 0
	...
	if (CheckEffect(aIndex, EFFECT_CHRISTMAS7) == 1) then
		AddBPPoint = 12
	...
	if(Class == CLASS_DW) then
		recoveryBP = AddBPPoint + (MaxBP * 3 / 100);
	elseif(Class == CLASS_DK) then
		recoveryBP = AddBPPoint + (MaxBP * 5 / 100);
	elseif(Class == CLASS_FE) then
		recoveryBP = AddBPPoint + (MaxBP * 3 / 100);
	elseif(Class == CLASS_MG) then
		recoveryBP = AddBPPoint + (MaxBP * 3 / 100);
	...
	if(mode == 0) then
		if((GetTickCount()-lpObj.BPAutoRecuperationTime)>5000) then
			recoveryBP = recoveryBP + ((MaxBP * 3 / 100));
		end
	end
	
	recoveryBP = recoveryBP + (MaxBP * (lpObj.BPRecoveryRate + lpObj.EffectOption_AddBPRecoveryRate) / 100);
	recoveryBP = recoveryBP + lpObj.BPRecovery + lpObj.EffectOption_AddBPRecovery;
